$(document).ready(function() {
	...
	$.getJSON('/backend/disease/' + disease['id'] + '/ontology_graph', function(data) {
  		let cy = create_cytoscape_vis("cy", layout, graph_style, data, null, false, "diseaseOntology");
...
function create_disease_table(data) {
    ...
    let htpDatatable = [];
    let htpGenes = {};
    let computationalDatatable = [];
    let computationalGenes = {};
	for (let i=0; i < data.length; i++) {
        let type = data[i].annotation_type;
...
function create_or_hide_table(tableData, options, tableIdentifier, doName, doLink, doId, annotationType, originalData) {
    if (tableData.length) {
        let localOptions =  $.extend({ aaData: tableData, oLanguage: { sEmptyTable: 'No genes annotated directly to ' + doName } }, options);
        let table = create_table(tableIdentifier, localOptions);
        ...
        let $parent = $("#" + tableIdentifier).parent();
        let emptyMessage = "There are no " + annotationType + " annotations for " + doName + ".";
