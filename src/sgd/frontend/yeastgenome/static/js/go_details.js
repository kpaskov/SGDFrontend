$(document).ready(function() {
  	...
  	$.getJSON('/backend/locus/' + locus['id'] + '/go_details', function(data) {
  	    let mc_bp_go_table = create_go_table("mc_bp", "No manually curated biological process terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "manually curated" && x["go"]["go_aspect"] == "biological process"}, data);
        create_download_button("mc_bp_go_table_download", mc_bp_go_table, locus['display_name'] + "_manual_bp_go");

        let mc_mf_go_table = create_go_table("mc_mf", "No manually curated molecular function terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "manually curated" && x["go"]["go_aspect"] == "molecular function"}, data);
        create_download_button("mc_mf_go_table_download", mc_mf_go_table, locus['display_name'] + "manual_mf_go");

        let mc_cc_go_table = create_go_table("mc_cc", "No manually curated cellular component terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "manually curated" && x["go"]["go_aspect"] == "cellular component"}, data);
        create_download_button("mc_cc_go_table_download", mc_cc_go_table, locus['display_name'] + "_manual_cc_go");

        let htp_bp_go_table = create_go_table("htp_bp", "No high-throughput biological process terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "high-throughput" && x["go"]["go_aspect"] == "biological process"}, data);
        create_download_button("htp_bp_go_table_download", htp_bp_go_table, locus['display_name'] + "_htp_bp_go");

        let htp_mf_go_table = create_go_table("htp_mf", "No high-throughput molecular function terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "high-throughput" && x["go"]["go_aspect"] == "molecular function"}, data);
        create_download_button("htp_mf_go_table_download", htp_mf_go_table, locus['display_name'] + "_htp_mf_go");

        let htp_cc_go_table = create_go_table("htp_cc", "No high-throughput cellular component terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "high-throughput" && x["go"]["go_aspect"] == "cellular component"}, data);
        create_download_button("htp_cc_go_table_download", htp_cc_go_table, locus['display_name'] + "_htp_cc_go");

        let comp_bp_go_table = create_go_table("comp_bp", "No computational biological process terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "computational" && x["go"]["go_aspect"] == "biological process"}, data);
        create_download_button("comp_bp_go_table_download", comp_bp_go_table, locus['display_name'] + "_computational_bp_go");

        let comp_mf_go_table = create_go_table("comp_mf", "No computational molecular function terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "computational" && x["go"]["go_aspect"] == "molecular function"}, data);
        create_download_button("comp_mf_go_table_download", comp_mf_go_table, locus['display_name'] + "_computational_mf_go");

        let comp_cc_go_table = create_go_table("comp_cc", "No computational cellular component terms for " + locus['display_name'], function(x) {return x["annotation_type"] == "computational" && x["go"]["go_aspect"] == "cellular component"}, data);
        ...
        let headers = ["Evidence ID", "Analyze ID", "", "Gene", "Gene Format Name", "Gene Ontology Term", "Gene Ontology Term ID", "Qualifier", "Aspect", "Method", "Evidence", "Source", "Assigned On", "Reference", "Relationships"];
  	...
  	$.getJSON('/backend/locus/' + locus['id'] + '/go_graph', function(data) {
        if(data['nodes'].length > 1) {
            let graph = create_cytoscape_vis("cy", layout, graph_style, data, null, false, "go");
