...
const GeneSequenceResources = React.createClass({
	...
	getResultTable4gene(data) {
		...
		_.map(genes, gene => {
		     ...
		     if (locus_type == 'ORF') {
		     	  ...
		     	  alignRow.push(<span style={ style.textFont }><br /><a href={ variantUrl } target='infowin2'>Variant Viewer</a><br /><a href={ strainUrl } target='infowin2'>Strain Alignment</a></span>);	
			  hasRow = hasRow + 1;		
		     }
		     else {
		...
		if (hasCoding > 0 && hasProtein > 0) { 
		     seqDLRow = [<span style={ style.textFont }><br />Sequence Downloads<br/>* DNA of Region<br/>* Coding Sequence of Selected ORF<br/>* Protein Translation of Selected ORF</span>];
		}
		else if (hasCoding > 0) {
	...
	getChrNodeLeft() {
		...
		var chr2num = { '-- choose a chromosome --': 0, 
		       ...
		       <input type='text' ref='start' name='start' onChange={this.onChange} size='105'></input>
		       to
                       <input type='text' ref='end' name='end' onChange={this.onChange} size='105'></input>
