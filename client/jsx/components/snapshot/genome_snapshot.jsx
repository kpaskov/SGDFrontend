...
const BarChart = require("../../components/viz/bar_chart.jsx");
const Legend = require("../../components/viz/legend.jsx");
const StandaloneAxis = require("../../components/viz/standalone_axis.jsx");
...
module.exports = React.createClass({
    ...
    toggleShowChromosomes: function (e) {
        ...
        let _clientTop = this.refs.wrapper.getBoundingClientRect().top;
    ...
    render: function () {
        let _buttonText = this.state.showChromosomes ?
            <span>Show by Genome&nbsp;<i className="fa fa-angle-up"></i></span> : <span>Show by Chromosome&nbsp;<i className="fa fa-angle-down"></i></span>;
        let buttonNode = (<a onClick={this.toggleShowChromosomes} className="button small chromosome-toggle" role="button">
            {_buttonText}
        </a>);
        ...
        if (this.state.showChromosomes) {
            let _maxFeatures = d3.max(this.props.data.chromosomes, (c) => {
                return c.features[0].value;
            });

            barNodes = _.map(this.props.data.chromosomes, (c) => {
                let chromosomeLabelNode = (<div className="clearfix chromosome-label-text">
            ...
            let _maxFeatures = d3.max(this.props.data.combined, (d) => { return d.value; })
    ...
    _getBarChart: function(data, maxY) {
        ...
        // init legend
        let totalOrfs = data[0].value;
        let legendElements = _.map(data[0].nestedValues, (entry, i) => {
            let _colors = ["#1f77b4", "#aec7e8", "#999"];
            let _percent = totalOrfs > 0 ? Math.round(entry.value/totalOrfs * 100) : "n/a";
            return {
                text: `${_percent}% ${entry.name}`,
                color: _colors[i]
            };
        });
        let legendNode = <div style={{ marginLeft: `${_labelRatio * 100}%`, fontSize: 12 }}><Legend elements={legendElements} labelText={`${totalOrfs.toLocaleString()} ORFs`} /></div>;

        let barNode = (<BarChart
        ...
        let axisNode = (<div style={{ height: 53 }}>
