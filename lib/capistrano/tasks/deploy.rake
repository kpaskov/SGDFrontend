namespace :deploy do
  ...
  task :write_config do
    on roles(:app), in: :sequence do
      config_file_content = ""
      ["HERITAGE_URL", "BACKEND_URL", "SECRET_KEY", "SENDER", "AUTHOR_RESPONSE_FILE", "COMPUTE_URL", "LOG_DIRECTORY", "ELASTICSEARCH_ADDRESS", "GOOGLE_CALENDAR_API_URL", "ENV", "CURATE_QA_URL", "CURATE_PROD_URL"].each do |key|
